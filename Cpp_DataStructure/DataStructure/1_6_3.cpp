...
public:
	cir_list() : n(0)
	{
		head = new node{ NULL, NULL, NULL }; //모두 NULL로 구성된 더미 노드 //첫번째 노드와 마지막 노드 사이의 더미 노드
		head->next = head;
		head->prev = head;
	}
	...
	{
		//아래 코드는 원소를 역순으로 삽입하지만, 원형 리스트이기 때문에 문제가 없다.
...
struct playlist
{
	cir_list<int> list; //편의상 음악 ID에 해당하는 정수 값을 사용
